...
export default function MealPlanPage() {
  ...
  useEffect(() => {
    async function init() {
      ...
      // ── Check DB for admin-edited plan (takes priority over localStorage) ──
      try {
        const res = await fetch('/api/user/plan?feature=meal_plan', {
          headers: { 'x-user-id': u.id }
        })
        const dbData = await res.json()
        if (dbData.plan) {
          // DB plan exists — use it (may have admin edits)
          const p = dbData.plan
          ...
          setGenerated(true)
          // Update localStorage with latest DB version
          localStorage.setItem('bodyfitai_meal_plan', JSON.stringify(p))
          return
        }
      } catch { /* fall through to localStorage */ }

      // ── Fall back to localStorage if no DB plan ──
