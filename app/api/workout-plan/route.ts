...
export default function WorkoutPlanPage() {
  ...
  useEffect(() => {
    async function init() {
      ...
      try {
        const res = await fetch('/api/user/plan?feature=workout_plan', {
          headers: { 'x-user-id': u.id }
        })
        const dbData = await res.json()
        if (dbData.plan?.plan) {
          setPlan(dbData.plan.plan)
          localStorage.setItem('bodyfitai_workout_plan', JSON.stringify(dbData.plan))
          setAuthChecked(true)
          return
        }
